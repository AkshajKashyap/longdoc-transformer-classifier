...
| `chunked_transformer_arxiv` | `arxiv` | 0.1800 | 0.0848 | model=prajjwal1/bert-tiny, chunk_selection=idf_top_k, aggregation=mean_proba, max_chunks_per_doc=8, chunk_size=220, overlap=40 |
| `long_context_transformer_arxiv` | `arxiv` | 0.1000 | 0.0202 | model=allenai/longformer-base-4096, max_length=1024, freeze_encoder=True, trainable_params=599051, gradient_accumulation=1 |
...
| `chunked_transformer_arxiv` | Chunked Transformer | `arxiv` | 0.1800 | 0.0848 | 100 | 50 | prajjwal1/bert-tiny | Chunks inherit weak document labels and may lack class evidence. | Structurally sees more text, but uses weak inherited chunk labels. |
| `long_context_transformer_arxiv` | Long-context Transformer | `arxiv` | 0.1000 | 0.0202 | 20 | 10 | allenai/longformer-base-4096 | Long-context transformers still have a maximum context window. | Uses a transformer architecture designed for longer contexts, avoiding manual chunk aggregation but still bounded by max_length and compute. |
